...
public class PedidoEventConsumer {
     ...
     * O método é anotado com @KafkaListener para configurar o consumo de mensagens.
     * Utiliza acknowledgment manual para garantir que as mensagens só sejam confirmadas
     * após o processamento bem-sucedido.
     *
     * @param event O evento de pedido criado recebido do Kafka
     * @param ack O objeto Acknowledgment para confirmar o processamento da mensagem
     */
    @KafkaListener(
        topics = "${app.kafka.topic.pedido-criado:pedido-criado}",
        groupId = "pedido-processor-group"
    )
    public void consumePedidoCriadoEvent(PedidoCriadoEvent event, Acknowledgment ack) {
        ...
        try {
            ...
            // Commit manual da mensagem após processamento bem-sucedido
            ack.acknowledge();
            logger.info("Commit realizado para o pedido: {}", event.getPedido().getId());
        } catch (Exception e) {
            ...
            // Não faz commit em caso de erro, permitindo reprocessamento
        }
    }
     ...
     * Utiliza acknowledgment manual para garantir que as mensagens só sejam confirmadas
     * após o envio bem-sucedido da notificação.
     *
     * @param event O evento de pedido criado recebido do Kafka
     * @param ack O objeto Acknowledgment para confirmar o processamento da mensagem
     */
    ...
    public void sendPedidoNotification(PedidoCriadoEvent event, Acknowledgment ack) {
        ...
        try {
            ...
            // Commit manual da mensagem após envio bem-sucedido da notificação
            ack.acknowledge();
            logger.info("Commit realizado para a notificação do pedido: {}", event.getPedido().getId());
        } catch (Exception e) {
            logger.error("Erro ao enviar notificação para o pedido: {}", e.getMessage(), e);
            // Não faz commit em caso de erro, permitindo reprocessamento
        }
    }
}
